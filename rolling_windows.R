# rolling_windows.R
# -------------------------------------------------------------------------
# Functions to be used in Assignment 2: U.S. COVID Trends
...
# Get the most recent national data from the New York Times
# TOD)
load_national_data <- function() {
  filename <- "https://raw.githubusercontent.com/nytimes/covid-19-data/master/us.csv"
  df <- read.csv(filename)
  return(df)
}

# Get the most recent state data from the New York Times
...
#       this function.
#
# To learn about`stop()` and `warning()` see: 
# https://adv-r.hadley.nz/conditions.html
#
moving_avg_counts <- function(national, window_size = 7) {

  # Check that this data frame has been initialized 
  if (is.null(national)) {
    stop("INFO-201: A2: The national data frame must be intitialized.")
  }
  ...
  # (Later we will learn how to "throw an error message" and stop.)
  if (window_size > nrow(national)) {
    warning("INFO-201: A2: Setting window_size to length of data frame.")
...
plot_moving_avg_cases <- function(national, window_size) {
  ...
  plot_alt <- "Daily US COVID Cases from January 2020 - Ocober 22. Data from New York Times."
  plot_caption <- paste0(
